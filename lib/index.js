...
// Import new modules
const stringUtils = require("./modules/stringUtils");
...
const functionUtils = require("./modules/functionUtils");
const systemUtils = require("./modules/systemUtils");
const networkUtils = require("./modules/networkUtils");
  ...
  Mp3Cutter: async (buffer, start, end) => {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (!fs.existsSync("mp3cut")) fs.writeFileSync("mp3cut", buffer);
        let src = path.join(__dirname, "mp3cut");
        let target = path.join(__dirname, "mp3cutf");
